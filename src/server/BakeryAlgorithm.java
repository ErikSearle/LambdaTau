...
public class BakeryAlgorithm {
    ...
    public void bakery(int tokens) {         //token selection and wait algorithm
        c[tokens] = true;                    //tells the other processes that a token is being made in the position
        if (d == false) {               //determines direction of the token, in this case positive
            tk[tokens] = (findMax() + 1);    //increases the max token value and places it in the array
            c[tokens] = false;               //tells the other processes that the allocation is finished
            for (int j = 0; j < n; j++) {//goes through the array
                while (c[j] == true) {
                    System.out.print("");
                }                       //waits on any processes entering the queue
                while ((tk[j] > 0 && (tk[j] < tk[tokens] || (tk[j] == tk[tokens] && j < tokens))) || (tk[j] < 0 && d == false)) {
                    System.out.print("");
                }
            }
            ...
            for (int j = 0; j < n; j++) {
                ...
                while ((tk[j] < 0 && (tk[j] > tk[tokens] || (tk[j] == tk[tokens] && j > tokens))) || (tk[j] > 0 && d == true)) {
                    System.out.print("");
                }
            }
            if (d == true) {
