...
public class AdminClient {
     ...
     */
    public AdminClient(int port, String password) throws IOException {
        socket = new Socket(InetAddress.getLocalHost(), port);
        input = new InputStreamReader(socket.getInputStream());
        output = new OutputStreamWriter(socket.getOutputStream());
        encryptor = Encryptor.negotiateKeysClientSide(input, output);
        char[] value = receive().toCharArray();
     ...
     */
    public AdminClient(InetAddress address, int port, String password) throws IOException {
